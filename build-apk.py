...
def modify_and_build(config):
    ...
    build_apk(channel, config['signing_congfigs'])
    if check_job_state():
...
def build_apk(channel, signing_congfigs):
    store_name = signing_congfigs['store_name']
    key_alias = signing_congfigs['key_alias']
    key_pwd = signing_congfigs['key_pwd']
    store_pwd = signing_congfigs['store_pwd']
